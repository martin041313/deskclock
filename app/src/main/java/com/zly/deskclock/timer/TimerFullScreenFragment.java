...
import com.zly.deskclock.CircleButtonsLayout;
import com.zly.deskclock.DeskClock;
import com.zly.deskclock.DeskClockFragment;
import com.zly.deskclock.LabelDialogFragment;
import com.zly.deskclock.LogUtils;
...
import com.zly.deskclock.widget.CircleView;
import com.zly.deskclock.widget.sgv.GridAdapter;
import com.zly.deskclock.widget.sgv.StaggeredGridView;
...
public class TimerFullScreenFragment extends DeskClockFragment
    ...
    private class TimersListAdapter extends GridAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            boolean drawRed = o.mState != com.zly.deskclock.timer.TimerObj.STATE_RESTART;
        ...
        public void addTimer(com.zly.deskclock.timer.TimerObj t) {
            mTimers.add(0, t);
            sort();
        }

        public void onSaveInstanceState(Bundle outState) {
            com.zly.deskclock.timer.TimerObj.putTimersInSharedPrefs(mPrefs, mTimers);
        }

        public void onRestoreInstanceState(Bundle outState) {
            com.zly.deskclock.timer.TimerObj.getTimersFromSharedPrefs(mPrefs, mTimers);
            sort();
        }
        ...
        private final Comparator<com.zly.deskclock.timer.TimerObj> mTimersCompare = new Comparator<com.zly.deskclock.timer.TimerObj>() {
            static final int BUZZING = 0;
            static final int IN_USE = 1;
            static final int NOT_USED = 2;

            protected int getSection(com.zly.deskclock.timer.TimerObj timerObj) {
            ...
            @Override
            public int compare(com.zly.deskclock.timer.TimerObj o1, TimerObj o2) {
    ...
    private class TimesUpListAdapter extends TimersListAdapter {
        ...
        public void onRestoreInstanceState(Bundle outState) {
            ...
            com.zly.deskclock.timer.TimerObj.getTimersFromSharedPrefs(mPrefs, mTimers, com.zly.deskclock.timer.TimerObj.STATE_TIMESUP);

            if (getCount() == 0) {
                mOnEmptyListListener.onEmptyList();
            } else {
                Collections.sort(mTimers, new Comparator<com.zly.deskclock.timer.TimerObj>() {
                    @Override
                    public int compare(com.zly.deskclock.timer.TimerObj o1, TimerObj o2) {
                        return (o1.mTimeLeft < o2.mTimeLeft) ? -1 : 1;
                    }
    ...
    private final Runnable mClockTick = new Runnable() {
        ...
        public void run() {
            ...
            for (int i = 0; i < mAdapter.getCount(); i++) {
                com.zly.deskclock.timer.TimerObj t = mAdapter.getItem(i);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (bundle != null && bundle.containsKey(com.zly.deskclock.timer.Timers.TIMESUP_MODE)) {
            if (bundle.getBoolean(com.zly.deskclock.timer.Timers.TIMESUP_MODE, false)) {
                try {
    ...
    private void resetTimer(com.zly.deskclock.timer.TimerObj t) {
        ...
        updateTimersState(t, com.zly.deskclock.timer.Timers.RESET_TIMER);
    ...
    public void updateAllTimesUpTimers() {
        ...
        LinkedList<com.zly.deskclock.timer.TimerObj> timesupTimers = new LinkedList<>();
        for (int i = 0; i < mAdapter.getCount(); i++) {
            com.zly.deskclock.timer.TimerObj timerObj = mAdapter.getItem(i);
            if (timerObj.mState == com.zly.deskclock.timer.TimerObj.STATE_TIMESUP) {
        ...
        while (timesupTimers.size() > 0) {
            final com.zly.deskclock.timer.TimerObj t = timesupTimers.remove();
            onStopButtonPressed(t);
        }

        if (notifyChange) {
            mPrefs.edit().putBoolean(com.zly.deskclock.timer.Timers.REFRESH_UI_WITH_LATEST_DATA, true).apply();
        }
    }
    ...
    private void onClickHelper(ClickAction clickAction) {
        switch (clickAction.mAction) {
            case ClickAction.ACTION_DELETE:
                final com.zly.deskclock.timer.TimerObj t = clickAction.mTimer;
                if (t.mState == com.zly.deskclock.timer.TimerObj.STATE_TIMESUP) {
                ...
                updateTimersState(t, com.zly.deskclock.timer.Timers.DELETE_TIMER);
    ...
    private void onPlusOneButtonPressed(com.zly.deskclock.timer.TimerObj t) {
        switch (t.mState) {
            case com.zly.deskclock.timer.TimerObj.STATE_RUNNING:
                ...
                updateTimersState(t, com.zly.deskclock.timer.Timers.TIMER_UPDATE);
            ...
            case com.zly.deskclock.timer.TimerObj.STATE_TIMESUP:
                ...
                updateTimersState(t, com.zly.deskclock.timer.Timers.START_TIMER);
    ...
    private void onStopButtonPressed(com.zly.deskclock.timer.TimerObj t) {
        switch (t.mState) {
            case com.zly.deskclock.timer.TimerObj.STATE_RUNNING:
                ...
                updateTimersState(t, com.zly.deskclock.timer.Timers.STOP_TIMER);
    ...
    private void onLabelPressed(com.zly.deskclock.timer.TimerObj t) {
    ...
    private void updateTimersState(com.zly.deskclock.timer.TimerObj t, String action) {
    ...
    private void updateTimesUpMode(com.zly.deskclock.timer.TimerObj timerObj) {
        if (mOnEmptyListListener != null && timerObj.mState != com.zly.deskclock.timer.TimerObj.STATE_TIMESUP) {
    ...
    public void onFabClick(View view) {
        if (mLastVisibleView != mTimersListPage) {
            ...
            com.zly.deskclock.timer.TimerObj t = new TimerObj(timerLength * DateUtils.SECOND_IN_MILLIS, getActivity());
            t.setState(com.zly.deskclock.timer.TimerObj.STATE_RUNNING);
            mAdapter.addTimer(t);
            updateTimersState(t, com.zly.deskclock.timer.Timers.START_TIMER);
            Events.sendTimerEvent(R.string.action_start, R.string.label_deskclock);
