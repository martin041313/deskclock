...
package com.zly.deskclock.timer;
...
import com.zly.deskclock.AnimatorUtils;
import com.zly.deskclock.DeskClock;
import com.zly.deskclock.DeskClockFragment;
import com.zly.deskclock.R;
import com.zly.deskclock.TimerSetupView;
import com.zly.deskclock.Utils;
import com.zly.deskclock.VerticalViewPager;
import com.zly.deskclock.events.Events;
...
public class TimerFragment extends DeskClockFragment implements OnSharedPreferenceChangeListener {
    ...
    private com.zly.deskclock.timer.TimerFragmentAdapter mAdapter;
    ...
    private final Runnable mClockTick = new Runnable() {
        ...
        public void run() {
            ...
            for (int i = 0; i < mAdapter.getCount(); i++) {
                final com.zly.deskclock.timer.TimerObj t = mAdapter.getTimerAt(i);
                if (t.mState == com.zly.deskclock.timer.TimerObj.STATE_RUNNING || t.mState == com.zly.deskclock.timer.TimerObj.STATE_TIMESUP) {
                    ...
                    if (t.mView != null) {
                        ...
                        if (toggle) {
                            ...
                            final boolean canAddMinute = com.zly.deskclock.timer.TimerObj.MAX_TIMER_LENGTH - t.mTimeLeft
                                    > com.zly.deskclock.timer.TimerObj.MINUTE_IN_MILLIS;
                            addMinuteButton.setEnabled(canAddMinute);
                        }
                    }
                }
                if (t.mTimeLeft <= 0 && t.mState != com.zly.deskclock.timer.TimerObj.STATE_RESTART) {
                    t.setState(com.zly.deskclock.timer.TimerObj.STATE_TIMESUP);
                    if (t.mView != null) {
                        t.mView.timesUp();
                    }
                }
                ...
                if (toggle && t.mView != null) {
                    if (t.mState == com.zly.deskclock.timer.TimerObj.STATE_TIMESUP) {
                        t.mView.setCircleBlink(mVisible);
                    }
                    if (t.mState == com.zly.deskclock.timer.TimerObj.STATE_STOPPED) {
    ...
    public void onResume() {
        ...
        if (mAdapter == null) {
            mAdapter = new com.zly.deskclock.timer.TimerFragmentAdapter(getChildFragmentManager(), mPrefs);
        }
        ...
        if (mPrefs.getBoolean(com.zly.deskclock.timer.Timers.REFRESH_UI_WITH_LATEST_DATA, false)) {
            // Clear the flag indicating the adapter is out of sync with the database.
            mPrefs.edit().putBoolean(com.zly.deskclock.timer.Timers.REFRESH_UI_WITH_LATEST_DATA, false).apply();
        }
        ...
        if (newIntent != null
                && newIntent.getBooleanExtra(com.zly.deskclock.timer.TimerFullScreenFragment.GOTO_SETUP_VIEW, false)) {
            goToSetUpView = true;
        } else if (newIntent != null
                && newIntent.getBooleanExtra(com.zly.deskclock.timer.Timers.FIRST_LAUNCH_FROM_API_CALL, false)) {
            ...
            final int timerPosition = ((com.zly.deskclock.timer.TimerFragmentAdapter) mViewPager.getAdapter())
                    .getTimerPosition(newIntent.getIntExtra(com.zly.deskclock.timer.Timers.SCROLL_TO_TIMER_ID, 0));
            ...
            newIntent.putExtra(com.zly.deskclock.timer.Timers.FIRST_LAUNCH_FROM_API_CALL, false);
        } else {
            if (mViewState != null) {
    ...
    private void updateTimerState(com.zly.deskclock.timer.TimerObj t, String action) {
     ...
     */
    private void updateTimerState(com.zly.deskclock.timer.TimerObj t, String action, boolean update) {
        if (com.zly.deskclock.timer.Timers.DELETE_TIMER.equals(action)) {
        ...
        i.setAction(action);
        i.putExtra(com.zly.deskclock.timer.Timers.TIMER_INTENT_EXTRA, t.mTimerId);
        i.putExtra(com.zly.deskclock.timer.Timers.UPDATE_NEXT_TIMESUP, update);
        // Make sure the receiver is getting the intent ASAP.
        i.addFlags(Intent.FLAG_RECEIVER_FOREGROUND);
        getActivity().sendBroadcast(i);
    }

    private void setTimerViewFabIcon(com.zly.deskclock.timer.TimerObj timer) {
        ...
        if (mLastView != mTimerView) {
            ...
            final AnimatorListenerAdapter adapter = new AnimatorListenerAdapter() {
                ...
                public void onAnimationStart(Animator animation) {
                    ...
                    final com.zly.deskclock.timer.TimerObj timerObj = new TimerObj(timerLength * DateUtils.SECOND_IN_MILLIS,
                        getActivity());
                    timerObj.setState(com.zly.deskclock.timer.TimerObj.STATE_RUNNING);
                    Events.sendTimerEvent(R.string.action_create, R.string.label_deskclock);

                    updateTimerState(timerObj, com.zly.deskclock.timer.Timers.START_TIMER);
            ...
            final com.zly.deskclock.timer.TimerObj t = getCurrentTimer();
            switch (t.mState) {
                case com.zly.deskclock.timer.TimerObj.STATE_RUNNING:
                    // Stop timer and save the remaining time of the timer
                    t.setState(com.zly.deskclock.timer.TimerObj.STATE_STOPPED);
                    t.mView.pause();
                    t.updateTimeLeft(true);
                    updateTimerState(t, com.zly.deskclock.timer.Timers.STOP_TIMER);
                ...
                case com.zly.deskclock.timer.TimerObj.STATE_DELETED:
                    ...
                    t.setState(com.zly.deskclock.timer.TimerObj.STATE_RUNNING);
                    t.mStartTime = Utils.getTimeNow() - (t.mOriginalLength - t.mTimeLeft);
                    t.mView.start();
                    updateTimerState(t, com.zly.deskclock.timer.Timers.START_TIMER);
                    Events.sendTimerEvent(R.string.action_start, R.string.label_deskclock);
                    break;
                case com.zly.deskclock.timer.TimerObj.STATE_TIMESUP:
                    if (t.mDeleteAfterUse) {
                        ...
                        // timer
                        t.setState(com.zly.deskclock.timer.TimerObj.STATE_DELETED);
                        updateTimerState(t, com.zly.deskclock.timer.Timers.DELETE_TIMER);
                        Events.sendTimerEvent(R.string.action_delete, R.string.label_deskclock);
                    } else {
                        t.setState(com.zly.deskclock.timer.TimerObj.STATE_RESTART);
                        ...
                        updateTimerState(t, com.zly.deskclock.timer.Timers.RESET_TIMER);
                        cancelTimerNotification(t.mTimerId);
    ...
    private com.zly.deskclock.timer.TimerObj getCurrentTimer() {
        if (mViewPager == null) {
            return null;
        }
        final int currPage = mViewPager.getCurrentItem();
        if (currPage < mAdapter.getCount()) {
            com.zly.deskclock.timer.TimerObj o = mAdapter.getTimerAt(currPage);
            return o;
        } else {
            return null;
        }
    ...
    public void onLeftButtonClick(View view) {
        ...
        final com.zly.deskclock.timer.TimerObj timer = getCurrentTimer();
        if (timer == null) {
            return; // Prevent NPE if user click delete faster than the fade animation
        }
        if (timer.mState == com.zly.deskclock.timer.TimerObj.STATE_TIMESUP) {
    ...
    private void deleteTimer(com.zly.deskclock.timer.TimerObj timer) {
        // Tell receiver the timer was deleted, it will stop all activity related to the
        // timer
        timer.setState(com.zly.deskclock.timer.TimerObj.STATE_DELETED);
        updateTimerState(timer, com.zly.deskclock.timer.Timers.DELETE_TIMER);
        Events.sendTimerEvent(R.string.action_delete, R.string.label_deskclock);
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
        if (prefs.equals(mPrefs)) {
            if (key.equals(com.zly.deskclock.timer.Timers.REFRESH_UI_WITH_LATEST_DATA) && prefs.getBoolean(key, false)) {
    ...
    public void setLabel(com.zly.deskclock.timer.TimerObj timer, String label) {
        timer.mLabel = label;
        updateTimerState(timer, com.zly.deskclock.timer.Timers.TIMER_UPDATE, false);
        // Make sure the new label is visible.
        mAdapter.populateTimersFromPref();
    }

    public void onPlusOneButtonPressed(com.zly.deskclock.timer.TimerObj t) {
        switch (t.mState) {
            case com.zly.deskclock.timer.TimerObj.STATE_RUNNING:
                t.addTime(com.zly.deskclock.timer.TimerObj.MINUTE_IN_MILLIS);
                long timeLeft = t.updateTimeLeft(false);
                t.mView.setTime(timeLeft, false);
                t.mView.setLength(timeLeft);
                mAdapter.notifyDataSetChanged();
                updateTimerState(t, com.zly.deskclock.timer.Timers.TIMER_UPDATE);
            ...
            case com.zly.deskclock.timer.TimerObj.STATE_TIMESUP:
                ...
                t.mTimeLeft = t.mOriginalLength = com.zly.deskclock.timer.TimerObj.MINUTE_IN_MILLIS;
                t.mView.setTime(t.mTimeLeft, false);
